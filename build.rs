fn main() {
    ...
    // targets (rustc-link-arg-tests). Try the pkg-config names that mlua-sys
    // itself probes (Homebrew lua@5.4 ships lua54.pc, Debian lua5.4.pc).
    for pc in ["lua5.4", "lua54", "lua-5.4"] {
        if let Ok(lib) = pkg_config::Config::new().cargo_metadata(false).probe(pc) {
